...
var config = {
	app: "app",
	devTemp: "C:/\NEISTWWebsite/\code/\TWNewEgg.ECWeb_Mobile/\TWNewEgg.ECWeb_Mobile",
	temp: "./tmp",
	dest: "dest",
	cssfolder: "/css",
...
var checkIsDev = function(arrData){
	var argv = arrData.indexOf("--dev");

	if(argv > -1){
		config.temp = config.devTemp;
		config.cssfolder = "/Themes/2016/css";
	}

	console.log("[開發環境位置]：" + config.temp);
};
...
gulp.task('sass', function(){
	return gulp.src(config.app + '/scss/{*,*/,*/*/,*/*/*/}*.scss')
			   ...
			   .pipe(gulp.dest(config.temp + config.cssfolder));
});
...
gulp.task('dist:css', function(){
	return gulp.src(config.temp + config.cssfolder +'/*.css')
			   .pipe(cssminify())
